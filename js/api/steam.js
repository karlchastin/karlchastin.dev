...
export async function fetchSteamReview(url) {
  try {
    ...
    let rawDateString = "";
    const dateEl =
      doc.querySelector(".postedDate") || doc.querySelector(".posted");
    if (dateEl) {
      rawDateString = dateEl.textContent
        .replace("Posted:", "")
        .replace(/Updated:.*/i, "")
        .trim();
    } else {
      const dMatch =
        htmlText.match(
          /(\d{1,2}\s+[A-Za-z]{3}(?:,\s+\d{4})?\s+@\s+\d{1,2}:\d{2}[ap]m)/i,
        ) ||
        htmlText.match(
          /([A-Za-z]{3}\s+\d{1,2}(?:,\s+\d{4})?\s+@\s+\d{1,2}:\d{2}[ap]m)/i,
        );
    ...
    const hMatch = flatText.match(
      /([0-9,]+)\s*people found this review helpful/i,
    );
    if (hMatch) helpful = hMatch[1];
    const fMatch = flatText.match(
    ...
    doc.querySelectorAll(".commentthread_comment").forEach((comment) => {
      ...
      if (bdi) {
        ...
        const link =
          comment.querySelector(".commentthread_author_link") ||
      ...
      if (avatarEl) {
        ...
        let bestFrame =
          frameSrcs.find(
            (s) => s.includes("/animated/") || s.endsWith(".gif"),
          ) || frameSrcs[0];
        if (bestFrame) {
          if (bestFrame.startsWith("//")) bestFrame = "https:" + bestFrame;
          else if (bestFrame.startsWith("/"))
        ...
        let bestAvatar =
          rawAvatarSrcs.find(
            (s) => s.endsWith(".gif") || s.includes("/animated/"),
          ) || rawAvatarSrcs[0];
        if (bestAvatar) {
          if (bestAvatar.startsWith("//")) bestAvatar = "https:" + bestAvatar;
          else if (bestAvatar.startsWith("/"))
            bestAvatar = "https://steamcommunity.com" + bestAvatar;
          avatar = bestAvatar;
        }
      }
...
export const applyReviewCache = (data) => {
  ...
  $("steam-review-funny").textContent =
    `${data.funny} people found this review funny`;
...
export async function updateSteamData() {
  ...
  const applyCache = (newData, isHtmlScrape = false) => {
    ...
    if (cache.hours) {
      ...
      if (recentDisplay)
        recentDisplay.style.display = cache.hours.includes("0 hours")
          ? "none"
          : "flex";
    }
  ...
  Promise.all([
    ...
    .then(([coreData, statsData]) => {
      if (!coreData.error)
        applyCache({
          name: coreData.name,
          avatar: coreData.avatar,
          level: coreData.level,
          hours: coreData.hours,
          statusMsg: coreData.stateMessage,
          statusClass: coreData.onlineState,
        });
      if (!statsData.error)
        applyCache({
          gamesCount: statsData.gamesCount,
          badgesCount: statsData.badgesCount,
    ...
    .catch((e) => {
      if (!cache.statusMsg)
        applyCache({
          statusMsg: "Live status unavailable",
          statusClass: "offline",
        });
    });
    ...
    .then((htmlText) => {
      ...
      if (badgeEl) {
        safeOverrides.badgeSrc = badgeEl
          .querySelector(".favorite_badge_icon img")
          ?.getAttribute("src");
        safeOverrides.badgeName =
          badgeEl
            .querySelector(".favorite_badge_description .name")
            ?.textContent?.trim() ||
          badgeEl.querySelector(".name")?.textContent?.trim() ||
          "";
        safeOverrides.badgeXp =
          badgeEl
      ...
      if (reviewLink) {
        fetchSteamReview(reviewLink.href)
          .then((data) => {
            if (data) {
              setCache("steamReviewCache", data);
              applyReviewCache(data);
            } else hideReviewLoading(true);
          })
