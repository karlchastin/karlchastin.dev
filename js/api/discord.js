...
function updateDiscordUI(data) {
    ...
    if (data.activities) {
        data.activities.filter(a => a.type !== 4).forEach((activity, i) => {
            ...
            let smallImgHTML = isMusic ? `<div style="position:absolute; bottom:-4px; right:-4px; width:28px; height:28px; border-radius:50%; border:3px solid #1e1e1e; background:#fa243c; display:flex; align-items:center; justify-content:center; pointer-events:auto; cursor:default; z-index:10;"><svg width="50%" height="50%" viewBox="0 0 24 24" fill="#fff"><path d="M12 3v10.55c-.59-.34-1.27-.55-2-.55-2.21 0-4 1.79-4 4s1.79 4 4 4 4-1.79 4-4V7h4V3h-6z"/></svg></div>` : '';
    ...
    const renderSingleBox = (slot, act) => {
        ...
        if (!box) {
            ...
            box.style.cssText = 'background: rgba(0,0,0,0.2); padding: 16px; border-radius: 16px; display:flex; gap: 16px; align-items:center; width: 100%; height: 100%; box-sizing: border-box;';
        ...
        if (!imgContainer) {
            ...
            imgContainer.style.cssText = 'flex-shrink:0; position:relative; width:84px; height:84px;';
