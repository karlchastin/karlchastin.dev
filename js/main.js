...
function refreshDynamicCard(cardId, targetTab, isActiveGetter) {
  ...
  if (show) {
    ...
    const gap = parent
      ? parseFloat(window.getComputedStyle(parent).gap) || 15
      : 0;
  ...
  prefTabs.forEach((tab) => {
    tab.addEventListener("click", async (e) => {
      ...
      const oldContent = wrapper
        ? wrapper.querySelector(".pref-content.active")
        : document.querySelector(".pref-content.active");
      const oldTab = wrapper
        ? wrapper.querySelector(".pref-tab.active")
        : document.querySelector(".pref-tab.active");
