...
export function setupTabs() {
  ...
  tabs.forEach((link, idx) => {
    link.addEventListener("click", async (e) => {
      ...
      if (
        tabName === "setup" &&
        typeof window.resetPrefToSetup === "function"
      ) {
        window.resetPrefToSetup(true);
      }
      if (
        tabName === "game_statistics" &&
        typeof window.resetGameStats === "function"
      ) {
      ...
      allCards.forEach((card) => {
        ...
        if (finalShow) {
          ...
          const gap = parent
            ? parseFloat(window.getComputedStyle(parent).rowGap) ||
              parseFloat(window.getComputedStyle(parent).gap) ||
              15
            : 0;
          card.classList.add("hide-card");
          card.style.height = "0px";
      ...
      currentIndex = idx;
      syncBackgrounds(currentIndex);
      const allTabEffects = document.querySelectorAll(
        "#glass-active .tab-bg-effect",
      );
